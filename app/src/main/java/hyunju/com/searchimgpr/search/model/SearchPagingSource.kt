...
class SearchPagingSource(private val searchText: String, private val searchNetworkApi: SearchNetworkApi) :
    PagingSource<Int, SearchMergedData>() {

    @SuppressLint("CheckResult")
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SearchMergedData> {
        val page = params.key ?: DEFAULT_PAGE_INDEX

        return try {
            val reqParam: Map<String, String> = hashMapOf("page" to page.toString(),"query" to searchText, "size" to DEFAULT_PAGE_INDEX.toString())
    ...
    override fun getRefreshKey(state: PagingState<Int, SearchMergedData>): Int? {
        return null
    }
}